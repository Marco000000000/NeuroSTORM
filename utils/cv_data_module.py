...
from sklearn.model_selection import StratifiedKFold, train_test_split # <--- Aggiungi train_test_split
import numpy as np
# ... altri import esistenti (UCLA, ecc) ...
...
class fMRIDataModule(pl.LightningDataModule):
    def __init__(self, **kwargs):
        ...
        # Argomenti extra per CV (con default per compatibilità retroattiva)
        self.use_cv = kwargs.get('use_cv', False)
        self.num_folds = kwargs.get('num_folds', 5)
        self.fold_index = kwargs.get('fold_index', 0)
        
        # Setup path standard (solo se NON siamo in CV o se vogliamo salvare comunque)
        if not self.use_cv:
        ...
        # Chiamata setup (alcune versioni di PL lo vogliono qui, altre no, male non fa)
        self.setup() 

    # ... (metodi get_dataset, make_subject_dict rimangono uguali) ...
    ...
    def setup_kfold(self, subject_dict):
        """Logica specifica per Cross Validation con Split Interno (Train/Val/Test)"""
        # 1. Filtra soggetti in base alle classi
        subjects = []
        targets = []
        ...
        if len(subjects) == 0:
            raise ValueError("CV Setup: Nessun soggetto trovato.")

        # 2. Outer Loop: K-Fold (Separa il TEST set per questo fold)
        ...
        # Dati per il TEST (Intoccabili)
        test_subjs = subjects[test_idx]
        
        # Dati per TRAIN + VAL (da splittare ancora)
        X_train_val = subjects[train_val_idx]
        y_train_val = targets[train_val_idx]
        
        # 3. Inner Loop: Hold-out Validation (10% del train, ovvero ~8% del totale)
        # Questo serve per l'Early Stopping, così non "bruciamo" il Test set
        train_subjs_pure, val_subjs_pure, _, _ = train_test_split(
            X_train_val, 
            y_train_val, 
            test_size=0.10, # 10% del training diventa validation
            stratify=y_train_val, 
            random_state=self.hparams.seed
        )

        # Crea i dizionari finali
        train_dict = {s: clean_dict[s] for s in train_subjs_pure}
        val_dict = {s: clean_dict[s] for s in val_subjs_pure}
        test_dict = {s: clean_dict[s] for s in test_subjs} # Ora è diverso da val!
        
        print(f"\n⚡ [CV Fold {self.fold_index+1}/{self.num_folds}] Setup Distinto:")
        print(f"   Train: {len(train_dict)} | Val (Early Stop): {len(val_dict)} | Test (Score): {len(test_dict)}")
        
        # Ora ritorniamo 3 dizionari DIVERSI
    ...
    def setup(self, stage=None):
        ...
        if self.use_cv:
            ...
            # === VECCHIA LOGICA SPLIT FILE ===
            # (Qui lasci tutto il codice originale che leggeva/creava split.txt)
            # ... codice originale determine_split_randomly / load_split ...
            # Per brevità qui metto un placeholder, tu mantieni il tuo codice vecchio
        ...
        # Creazione Dataset (Uguale per entrambi i casi)
        ...
        # Logging solo se siamo sul rank 0 (per evitare spam in DDP)
        # print(f"Dataset Size -> Train: {len(self.train_dataset)}, Val: {len(self.val_dataset)}")
