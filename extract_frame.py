...
START_SEC = 3.2        # Offset iniziale (ritardo fMRI)
TOTAL_TR = 250         # ESATTAMENTE 250 volumi
...
final_path = os.path.join(DATA_DIR, "features_brain_target_250.pt")

for key in dataset:
    if len(dataset[key]) > 0:
        # Stack e pulizia dimensioni
        t = torch.cat(dataset[key], dim=0) # [250, 1, 1280]
        if len(t.shape) == 3 and t.shape[1] == 1:
            t = t.squeeze(1) # [250, 1280]
        dataset[key] = t
        print(f"  • {key}: {t.shape}")

# Controllo finale dimensione
assert dataset["clip_dominant"].shape[0] == 250, f"Errore: Generati {dataset['clip_dominant'].shape[0]} frame invece di 250!"
