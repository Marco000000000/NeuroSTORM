...
# ID del modello CLIP standard (ViT-Large produce embedding da 768)
...
# A. CLIP (Standard via Hugging Face)
clip_processor = CLIPProcessor.from_pretrained(CLIP_ID)
clip_model = CLIPVisionModelWithProjection.from_pretrained(CLIP_ID).to(DEVICE).eval()
...
def get_clip_emb(pil_image):
    """Estrae embedding CLIP [1, 768]"""
    inputs = clip_processor(images=pil_image, return_tensors="pt").to(DEVICE)
    with torch.no_grad():
        outputs = clip_model(**inputs)
        # image_embeds contiene la proiezione finale dello spazio visivo
        emb = outputs.image_embeds 
    return emb.cpu() # [1, 768]
...
final_path = os.path.join(DATA_DIR, "features_despicable_clip.pt")

for key in dataset:
    if len(dataset[key]) > 0:
        # Stack e pulizia dimensioni
        t = torch.cat(dataset[key], dim=0) # [750, 1, 768] (per CLIP) o [750, 1, 4096] (per AlexNet)
