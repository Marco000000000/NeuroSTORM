...
from utils.cv_data_module_hbn_clf import fMRIDataModule 
...
def cli_main():
    ...
    parser.add_argument("--dataset_name", type=str, choices=["HCP1200", "ABCD", "UKB", "Cobre", "ADHD200", "HCPA", "HCPD", "UCLA", "HCPEP", "HCPTASK", "GOD", "NSD", "BOLD5000", "MOVIE", "TransDiag", "HBN"], default="HCP1200")
    ...
    parser.add_argument("--num_folds", type=int, default=5, help="Number of Cross Validation folds (ignored when --loo is set)")
    parser.add_argument("--loo", action='store_true', help="Leave-One-Out CV: num_folds = number of subjects (overrides --num_folds)")
    ...
    if args.loo:
        print("\n🔍 LOO mode: probing dataset to count subjects...")
        _probe_dm = fMRIDataModule(use_cv=True, fold_index=0, **vars(args))
        n_folds = _probe_dm.n_loo_folds
        print(f"   LOO: {n_folds} subjects found → {n_folds} folds")
        del _probe_dm
    else:
    ...
    if not args.test_only:
        ...
        cv_type = "LOO" if args.loo else f"{n_folds}-Fold"
        print("\n" + "🏆"*20)
        print(f" {cv_type} CV COMPLETED")
        print("🏆"*20)
