...
RESULTS_FILE = "metrics_val_vs_test.csv"

DEVICE = "cuda" if torch.cuda.is_available() else "cpu"
INPUT_DIM = 2304
SEQ_LEN = 20
HRF_SHIFT = 14  # Shift confermato
...
def load_data_split(subject_id, split_name, video_targets):
    """
    Carica i dati per uno specifico split ('val' o 'test').
    """
    # Costruisce path: .../neurostorm_features_split/val/sub-XXX
    ...
    for fpath in files:
        try:
            ...
            fmri = torch.load(fpath, map_location='cpu').float().view(-1, SEQ_LEN)
...
def compute_metrics(preds, targets):
    ...
    similarity_matrix = torch.mm(preds_norm.half(), targets_norm.T.half())
...
def compute_generalization(preds, true_targets, train_pool):
    """Accuratezza contro distrattori del Training Set"""
    ...
    total_hits = 0
    total_comps = 0
    N_BOOTSTRAP = 500 # Un po' meno per velocità
...
def main():
    ...
    print(f"⚖️  Confronto VAL vs TEST per {args.subject} (Shift {HRF_SHIFT})")
    print(f"{'Variant':<15} | {'Split':<5} | {'2-Way':<8} | {'Top-1':<8} | {'Gen(vsTrain)':<12}")
    print("-" * 65)
    ...
    for var in variants:
        ...
        # Train Pool per Generalizzazione
        train_pool = targets[:int(targets.shape[0]*0.8)]
