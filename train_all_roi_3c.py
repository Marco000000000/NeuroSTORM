...
# === CONFIGURAZIONE RIGIDA MULTICLASSE ===
...
METADATA_PATH = "/home/mfinocchiaro/miccai2026/NeuroSTORM/data/neurostorm_input_4d/metadata/labels_3class.csv"  # Aggiornato al file con 3 classi
OUTPUT_DIR = "clinical_master_results_3class" # Nuova cartella di output
HRF_SHIFT = 0
...
def main():
    ...
    if not os.path.exists(METADATA_PATH):
        print(f"❌ Errore: File {METADATA_PATH} non trovato. Assicurati di aver generato le 3 classi.")
        return
    ...
    print(f"🚀 Avvio Pipeline NeuroSTORM su {len(subjects)} soggetti (3 Classi)...")
    ...
    for full_id in tqdm(subjects):
        ...
        with torch.no_grad():
            ...
            all_features.append({
                "subject_id": short_id,
                "label": int(meta_df.loc[short_id, 'label']),
    ...
    if len(control_ids) < 2:
        print("❌ Non ci sono abbastanza controlli per calcolare l'ISC.")
        return
    ...
    y_preds_rf = []
    print("\n⚖️  Classificazione Cross-Validation (Multiclasse)...")
    for train_idx, test_idx in loo.split(X):
        # Aggiunto class_weight='balanced' per le 3 classi
        clf = RandomForestClassifier(n_estimators=100, random_state=42, class_weight='balanced')
        clf.fit(X[train_idx], y[train_idx])
        y_preds_rf.append(clf.predict(X[test_idx])[0])

    # 6. Report e Matrice 3x3
    acc = accuracy_score(y, y_preds_rf)
    b_acc = balanced_accuracy_score(y, y_preds_rf)
    
    print("\n=== REPORT DI CLASSIFICAZIONE ===")
    print(f"Accuracy Totale: {acc:.2%}")
    print(f"Balanced Accuracy: {b_acc:.2%}")
    print("\nDettagli per classe:")
    print(classification_report(y, y_preds_rf, target_names=['Control', 'ADHD-I', 'ADHD-C']))

    cm = confusion_matrix(y, y_preds_rf, labels=[0, 1, 2])
    ...
    sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', 
                xticklabels=['Ctrl', 'ADHD-I', 'ADHD-C'], 
                yticklabels=['Ctrl', 'ADHD-I', 'ADHD-C'])
    plt.title(f"Confusion Matrix (Bal Acc: {b_acc:.2%})")
    plt.ylabel('True Label')
    plt.xlabel('Predicted Label')

    # Feature Importance
    plt.subplot(1, 2, 2)
    sns.barplot(x=['Rank', 'Stability', 'Engage', 'ISC'], y=clf.feature_importances_, palette='viridis')
    plt.title("Importanza Marker Diagnostici")
    
    plt.tight_layout()
    plt.savefig(f"{OUTPUT_DIR}/diagnostic_summary_3class.png")
    
    # Salvataggio CSV
    df.to_csv(f"{OUTPUT_DIR}/complete_adhd_stats_3class.csv", index=False)
    print(f"\n✅ Studio multiclasse completato. Risultati salvati in {OUTPUT_DIR}")
